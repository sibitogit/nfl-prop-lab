import pandas as pd
from core import grade_prop, record, hit_rate, line_explorer, consistency_metrics, defense_vs_position, research_summary
...
def test_research_summary_over():
    df=pd.DataFrame({"receiving_yards":[40,55,60,70]})
    g=grade_prop(df,"receiving_yards",50.0,"Over")
    lines=research_summary(g,"receiving_yards",50.0,"Over",last5=g)
    assert "3 wins in 4 games" in lines[0]
    assert "75% graded hit rate" in lines[0]
    assert "above the 50 line" in lines[1]

def test_research_summary_push_denominator():
    df=pd.DataFrame({"receptions":[4,5,6]})
    g=grade_prop(df,"receptions",5.0,"Over")
    lines=research_summary(g,"receptions",5.0,"Over")
    assert "50% graded hit rate" in lines[0]
    assert "1-1-1 W-L-P" in lines[0]
